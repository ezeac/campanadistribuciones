...
function crf_show_extra_profile_fields( $user ) {
    $cuil = get_the_author_meta( 'cuil', $user->ID );
    ?>
    <h3><?php esc_html_e( 'Información personal', 'crf' ); ?></h3>

    <table class="form-table">
        <tr>
            <th><label for="cuil"><?php esc_html_e( 'Cuil', 'crf' ); ?></label></th>
            <td>
                <input type="text" name="cuil" value="<?php echo esc_attr( $cuil ); ?>"/>
            </td>
        </tr>
    </table>
    <?php
...
function crf_update_profile_fields( $user_id ) {
    ...
    if ( ! empty( $_POST['cuil'] ) ) {
        update_user_meta( $user_id, 'cuil', intval( $_POST['cuil'] ) );
    }
}
...
function page_title_content() {
    if (is_shop() || is_product_category()) {
        ...
        if (isset($_REQUEST['filter_marca'])) {
            //echo $_REQUEST['filter_marca'];
            $orderby = 'name';
            $order = 'asc';
            $hide_empty = true;
            $cat_args = array(
            ...
            if( !empty($product_categories) ){
                    foreach ($product_categories as $key => $category) {
                        ...
                        $products = new WP_Query(array(
                            ...
                            'tax_query' => array(
                                array(
                                    'taxonomy' => 'pa_marca', // Product attribute taxonomy: always start with 'pa_'
                        ...
                        if($products->have_posts()){
                            ...
                            echo "<style>.cat-item-{$category->term_id} { display: none; }</style>";
                            //$category->term_id
                        }
                        //echo "</pre>";
                }
            ...
            <script>
                $(document).ready(function(){
                    $(".product-categories .cat-item").each(function(){
                        $(this).find("a").attr("href",$(this).find("a").attr("href")+"?filter_marca=<?php echo $_REQUEST['filter_marca']; ?>");
                    });
                });
            </script>
            <?php
...
function hide_prices() {
    if (!is_user_logged_in()) {
        ?>
        <style>
        .product_meta .sku-and-price .price-cont, .xoo-qv-main .price-cont, .woocommerce .product .price.price, .woocommerce .product .onsale.onsale, .button[name='add-to-cart'], .quantity {
